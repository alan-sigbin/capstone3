...
async function tokenHandler(token) {
    const res = await axios.post("/checkout", {token, donate});
        ...
        { appt.appt_client._id === auth._id && 
                    <div className="row">
                        <Card className="text-center mx-5 my-2 px-0 container-fluid" border="info">
                            ...
                            <Card.Body>
                                <Card.Title className="f12 text-left">{appt.appt_title}</Card.Title>
                                <Card.Text>
                            ...
                            <StripeCheckout 
                                ...
                                token={tokenHandler}
                                amount={donate.price * appt.appt_price}
                                name={donate.name}
                                billingAddress
                                shippingAddress
                            />
